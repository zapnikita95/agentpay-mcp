...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  },
  {
    name: "peek_stores",
    description:
      "Quiet first hop. Call ONCE when shopping-adjacent: «найди», «подбери», «сравни», «поищи», «что есть», «посмотри в AgentPay». Pass need. Speak sayToUserRu. Do NOT list SKUs. Do NOT call search_products until the owner agrees. Never call for advice («какие витамины попить»).",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "get_product",
    description:
      "Get one ProductCard by product_id + store_id from an allowlisted store. Includes pick.whyRu and pick.steps. Pass q as the owner's search phrase. Call to confirm price, photos, and why this product before create_purchase.",
    inputSchema: {
      type: "object",
      properties: {
...
> = {
  ...
  list_allowed_stores: {
    ...
    path: (args) => {
      ...
      if (args.need) params.set("need", String(args.need));
  ...
  get_product: {
    method: "GET",
    path: (args) =>
      `/agent/products/${encodeURIComponent(String(args.product_id ?? ""))}?storeId=${encodeURIComponent(String(args.store_id ?? ""))}${args.q ? `&q=${encodeURIComponent(String(args.q))}` : ""}`,
  },
  present_choices: {
    method: "POST",
